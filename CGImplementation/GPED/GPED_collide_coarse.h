#ifndef __GPED_COLLIDE_COARSE_H__
#define __GPED_COLLIDE_COARSE_H__
...
namespace GPED
{
	...
	public:
		 ...
		 */
		BoundingSphere(const glm::vec3& centre, real radius);
		 ...
		 * by to incoporate the given bounding sphere. Note that this
		 ...
		 * For a bouding sphere it is a simple calculation.
		 */
		real getSize() const
		{
			// 4/3 * pi * r^3
	...
	{
		/** 
		 * Holds the bodiesthat might be in contact.
		 */
		RigidBody* body[2];
	};
	...
	public:
		 ...
		 * with its associated
		 * rigid body and child node. This method deletes the node
		 * and all its children (but obviously not the rigid bodies). This
		 * also has the effect of deleting the sibling of thisnode, and
		 * changing the parent node so that it contains the data currently
		 * in that sibling. Finally it forces thehierarchy above the
	...
	protected:
		 ...
		 * that checks for overlapping with another object of itw own type.
		 */
		 ...
		 * given limi). Returns the number of potential contacts it 
		 * found.
		 */
		 ...
		 * based on the vounding volume of its children.
		 */
		...
		RigidBody* newBody, const BoundingVolumeClass& newVolume)
	{
		...
		// Otherwise we need to work out whilc child gets to keep
		// the inserted body. we give it to whoever would grow the least to incorporate it.
		else
		{
			if (children[0]->volume.getGrowth(newVolume) < children[1]->volume.getGrwoth(newVolume))
			{
	...
	BVHNode<BoundingVolumeClass>::~BVHNode()
	{
		...
		if (parent)
		{
			...
			// Recalculate the parent's boudning volume
	...
	) const
	{
		...
		// to report contacts
		if (!overlap(other) || limit == 0) return 0;
		...
		if (other->isLeaf() || (!isLeaf() && volume->getSize() >= other->volume->getSize()))
		{
			...
			// Check we have enough slots to do other side too
			if (limit > count)
			{
				return count + 
					children[1]->getPotentialContactsWith
		...
		else
		{
			...
			{
				return count +
					getPotentialContactsWith(other->children[1], contacts + count, limit);
			}
			else
			{
				return count;
